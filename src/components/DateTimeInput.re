...
let component = ReasonReact.reducerComponent("DateTimeInput");
  ...
  reducer: (action, state) =>
    ...
    | FinishedEditing =>
      ReasonReact.UpdateWithSideEffects(
        ...
        (
          self => {
            onChange(self.state.currentValue |> Date.fromTimestamp);
            onValidate(self.state.currentValue |> Date.fromTimestamp);
          }
        ),
      )
    | StartEditing => ReasonReact.Update({currentValue: state.currentValue})
    },
  initialState: () => {currentValue: value |> Date.toTimestamp},
  render: self => {
